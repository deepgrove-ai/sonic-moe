...
class TopK_bias:
    ...
    def __call__(
        ...
        self.kernel(mX, mXBias, mValues, mIndices, input_tv_layout, input_tiler_mn, output_tv_layout, output_tiler_mn).launch(
    ...
    def kernel(
        ...
        # bias load
        bias_reg = cute.make_fragment(256, cutlass.Float32)
        for i in cutlass.range(256, unroll_full=True):
            bias_reg[i] = mBias[i].to(cutlass.Float32)
        ...
        for i in cutlass.range_constexpr(self.k):
            idx = topk_indices[i]
            topk_vals[i] = topk_vals[i] - bias_reg[idx]
