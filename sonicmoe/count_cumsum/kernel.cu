...
inline __device__ void _compute_cumsum(typename BlockScan::TempStorage &temp_storage,
                                       int32_t *shared_memory,
                                       const uint32_t &E) {
...
__global__ void count_cumsum_cuda_kernel(
    const scalar_t *x, int32_t *count_output, int32_t *cumsum_output, const int64_t N, const uint32_t E) {
...
void count_cumsum_cuda(const torch::Tensor &x,
    ...
    const uint32_t block_reduce_smem_size =
        do_cumsum ? std::max(sizeof(uint32_t), sizeof(typename BlockScan::TempStorage)) : 0;

    DISPATCH_INT_KERNEL(
        x.scalar_type(), "count_cumsum_cuda_kernel", scalar_t, ([&] {
            cudaFuncSetAttribute(
                (do_cumsum ? count_cumsum_cuda_kernel<scalar_t, true> : count_cumsum_cuda_kernel<scalar_t, false>),
            ...
            cudaLaunchKernelEx(
                ...
                _cumsum_output.has_value() ? _cumsum_output.value().data_ptr<int32_t>() : nullptr,
                N,
                E);
        }));
}
